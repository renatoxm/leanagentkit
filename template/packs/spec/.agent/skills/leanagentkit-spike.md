---
name: leanagentkit-spike
description: Throwaway experiments to validate feasibility before a real build. Use when exploring "is this possible?", comparing approaches, or prototyping.
invocation: auto
---

# Skill: leanagentkit-spike

**Goal:** Feel out an idea before committing to a real build — validate
feasibility, compare approaches, or surface unknowns that no amount of
research will answer. Spikes are disposable by design. Throw them away once
...
- The work is production path — use `leanagentkit-new-spec` instead
- The idea is already validated — jump straight to `leanagentkit-grill` →
  `leanagentkit-new-spec` → `leanagentkit-implement-spec`
...
Use the host agent's search and terminal tools for research — read docs, check
what's installed, compare versions.
...
**Bias toward something the user can interact with.** Spikes fail when the
only output is a log line that says "it works." Default choices, in order:
...
**Comparison spikes (002a / 002b):** build them back to back, then do a
head-to-head comparison at the end.
...
back, then do a head-to-head comparison:
...
- **Validated spike?** Hand off to `leanagentkit-grill` → `leanagentkit-new-spec`
  with the verdict as evidence.
- **Spike code in production?** Wrong — spikes are throwaway. Start fresh with
  `leanagentkit-implement-spec` and `leanagentkit-tdd`.

## Red flags

- Building production code during a spike
- No observable output the user can interact with
- Declaring success after one happy-path run
- Skipping the verdict section
- Keeping spike code instead of throwing it away
- Spike when docs/code already answer the question

## Verification

- [ ] Spike question stated as Given/When/Then
- [ ] User aligned on spike order (if multi-spike)
- [ ] `spikes/NNN-name/README.md` exists with verdict
- [ ] Verdict is VALIDATED, PARTIAL, or INVALIDATED with evidence
- [ ] Recommendation for the real build is documented
- [ ] Spike code is isolated under `spikes/` (not mixed into production paths)

## Attribution

Adapted from the GSD (Get Shit Done) project's `/gsd-spike` workflow — MIT ©
2025 Lex Christopherson ([gsd-build/get-shit-done](https://github.com/gsd-build/get-shit-done)).
