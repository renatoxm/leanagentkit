...
If the request is empty, default to: _the workflow we just went through in this
conversation — review the steps taken and distill them into a reusable skill._
...
- When a gotcha is discovered later, append under `## Learned notes` in the
  generated skill. **Project-wide** avoidable mistakes (hooks, commitlint, repo
  conventions) go in `docs/memory/LEARNINGS.md` — not in skill Learned notes.

## When NOT to use

- One-off tasks unlikely to repeat — refresh ACTIVE_CONTEXT / optional finalize instead.
