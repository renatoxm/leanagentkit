...
**Daily loop:** `leanagentkit-start-session` → (`leanagentkit-grill` → `leanagentkit-new-spec` → `leanagentkit-decompose-spec` (optional) → `leanagentkit-implement-spec` for new work) → `leanagentkit-check` → `leanagentkit-end-session`
