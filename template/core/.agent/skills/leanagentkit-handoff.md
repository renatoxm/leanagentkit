...
without re-deriving state. **Explicit only** — do not ambient-auto handoff.
...
**Relationship to ambient memory / finalize:** Ambient touches and optional
`leanagentkit-end-session` persist _durable project memory_ (`ACTIVE_CONTEXT`,
`LEARNINGS`, `PROGRESS`, map). Handoff is the _cross-window / cross-tool baton_
for an in-flight task — use it when you are not done but the conversation can't
continue in place.

**Recommended workflow when context fills:**

1. Run this skill (`handoff`) while the conversation still has full context.
2. Optionally run `leanagentkit-end-session` if you also want `PROGRESS` / pack hooks.
3. User opens a **new chat** in the same or a different tool.
4. Next agent follows `AGENTS.md` §6 (read ACTIVE_CONTEXT, map, LEARNINGS, then
   HANDOFF) — or optional `leanagentkit-start-session` for pack hooks — and continues.
...
3. Add a **Suggested skills** section naming the kit skills the next agent should
   invoke (e.g. ambient §6 prime or `leanagentkit-start-session` for pack hooks,
   `leanagentkit-implement-spec` when resuming spec work, the active
...
4. **Do not duplicate** content already captured in other artifacts (specs,
   ADRs, `PROGRESS.md`, `LEARNINGS.md`, commits, diffs) — reference them by path instead.
...
- Nothing restated that already lives in a spec, ADR, `PROGRESS.md`, or LEARNINGS.
